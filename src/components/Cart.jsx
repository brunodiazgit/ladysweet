...
function Cart() {
    ...
    return (
        <div className="flex flex-col items-center min-h-screen gap-20 md:gap-14 pt-20">
            {cart.map(item => (
                <CartItem item={item} key={item.id} />
            ))}
            {cart.length ? <Link to={'/checkout'}>
                <button className=" mt-10 flex justify-center mb-10 md:mb-14 items-center w-56 h-20 text-2xl bg-pink-700 text-white rounded-xl hover:bg-pink-500">
